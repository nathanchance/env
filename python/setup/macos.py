...
def is_vm() -> bool:
    return 'VirtualMac' in lib.utils.chronic(['sysctl', 'hw.model']).stdout
...
def setup_ssh() -> None:
    ...
    if not (ssh_key := Path(home, '.ssh/id_ed25519')).exists():
        ...
        if not (keys_folder := Path('/tmp/keys')).exists():  # ruff:ignore[hardcoded-temp-file]
...
if __name__ == '__main__':
    if (user := get_home().name) != 'nathan':
        msg = f"Current user ('{user}') is unexpected!"
        raise RuntimeError(msg)

    if os.uname().sysname != 'Darwin':
        msg = 'Not running on macOS?'
        raise RuntimeError(msg)
