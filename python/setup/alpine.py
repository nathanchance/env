...
def enable_community_repo():
    conf, text = lib.root.path_and_text('/etc/apk/repositories')
...
def update_and_install_packages():
    ...
    lib.root.apk(['update'])
    lib.root.apk(['upgrade'])
    lib.root.apk(['add', *packages])


def setup_user(user_name, user_password):
    if not lib.root.user_exists(user_name):
        useradd_cmd = [
            'adduser',
            '--disabled-password',
        ...
        lib.root.chpasswd(user_name, user_password)
...
def setup_podman(user_name):
    ...
    modules, modules_text = lib.root.path_and_text('/etc/modules')
    ...
    lib.root.podman_setup(user_name)
...
if __name__ == '__main__':
    ...
    lib.root.setup_initial_fish_config(args.user_name)
