...
function user_setup -d "Setup a user account, downloading all files and placing them where they need to go"
    ...
    switch $LOCATION
        case aadp chromebox framework-desktop honeycomb mac-studio test-desktop-amd-8745HS test-desktop-intel-{11700,n100} test-laptop-intel
...
rpmbuild/' >>$gitignore
    ...
    if command -q modprobed-db
        modprobed-db
        modprobed-db store
        cp $HOME/.local/share/modprobed-db/modprobed.db{,.orig}
        begin
            cat $HOME/.local/share/modprobed-db/modprobed.db.orig
            string join \n {vhost_,}vsock
        end | path sort -u >$HOME/.local/share/modprobed-db/modprobed.db
        rm $HOME/.local/share/modprobed-db/modprobed.db.orig
    ...
    # Configuration files (vim, tmux, terminal profiles, etc)
    setup_config_links

    # Graphical application setup
    if set -q DISPLAY
        if __is_installed gnome-terminal
            dconf load $gnome_prof <$ENV_FOLDER/configs/local/Nathan.dconf
        end
