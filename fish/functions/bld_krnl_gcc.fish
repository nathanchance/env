...
function bld_krnl_gcc -d "Builds the latest GCC suitable for building the Linux kernel"
    if test -z "$BINUTILS"
        set BINUTILS 2.37
    end
    if test -z "$GCC"
...
PREFIX=$PREFIX" >$bld/config
    ...
    set --path bld_krnl_gcc_path
    for val in $PATH
        if not string match -q -r /mnt/c/ $val
            set -a bld_krnl_gcc_path $val
        end
    end

    for target in $targets
        rm -rf $bld/$target
        PATH="$bld_krnl_gcc_path" $bld/build $bld_opts $target
    end
end
