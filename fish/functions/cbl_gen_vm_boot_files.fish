...
function cbl_gen_vm_boot_files -d "Generate files needed to boot local compiled kernels with cbl_vmm.py"
    ...
    # Make sure all modules the virtual machine might need are loaded (virtiofs, etc)
    ...
    if test -e $HOME/.local/share/modprobed-db/modprobed.db
        cat $HOME/.local/share/modprobed-db/modprobed.db
    else
        lsmod
