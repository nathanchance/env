...
function clone_repo_from_bundle -d "Clone repo using a clone bundle if possible"
    set num_args (count $argv)
    if test $num_args -lt 2; or test $num_args -gt 3
        print_error "$(status function) <repo> <dest> (<existing_bundle>)"
        return 1
    end
    ...
    if not set -q bundle
        if test $num_args -eq 3
            ...
            if not test -e $bundle
                print_warning "Provided bundle ('$bundle') does not existing, cloning via $url..."
                set -e bundle
            end
        end
    end
